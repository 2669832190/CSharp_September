namespace 飞行棋 {
	class Program {
		...
		static bool RandomMove(int w , int h , ref Player p , ref Player otherP , Map map) {
			...
			if ( otherP.isPause ) {
				Console.SetCursorPosition(2 , h - 5);
				Console.Write("{0}处于暂停状态，{1}需要暂停一回合" , p.type == E_PlayerType.Player ? "你" : "电脑", p.type == E_PlayerType.Player ? "你" : "电脑");
				Console.SetCursorPosition(2 , h - 4);
				Console.Write("请按任意键，让{0}开始扔色子" , p.type == E_PlayerType.Player ? "电脑" : "你");
				//停止暂停
				otherP.isPause = false;
				return false;
			}
			...
			if ( p.nowIndex >= map.grids.Length - 1 ) {
				...
				switch ( grid.type ) {
					...
					case E_Grid_Type.Pause:
						...
						p.isPause = true;
						Console.SetCursorPosition(2 , h - 4);
						Console.Write("{0}进入传送门，{1}将暂停一回合" , p.type == E_PlayerType.Player ? "你" : "电脑" , p.type == E_PlayerType.Player ? "电脑" : "你");
						Console.SetCursorPosition(2 , h - 3);
						Console.Write("请按任意键，让{0}开始扔色子" , p.type == E_PlayerType.Player ? "你" : "电脑");
						break;
					case E_Grid_Type.Rocket:
						...
						Console.SetCursorPosition(2 , h - 4);
						Console.Write("{0}驾驶火箭，前进3步" , p.type == E_PlayerType.Player ? "你" : "电脑");
						Console.SetCursorPosition(2 , h - 3);
						Console.Write("请按任意键，让{0}开始扔色子" , p.type == E_PlayerType.Player ? "电脑" : "你");
						break;
				}
		...
		public Map(int x , int y , int num) {
			...
			for ( int i = 0; i < num; i++ ) {
				...
				//有5%的几率 是时空隧道
					...
					grids[i].type = E_Grid_Type.Rocket;
				}
