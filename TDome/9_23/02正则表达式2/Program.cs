    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            */
            ...
            /*题1:验证身份证号 18位 最后一位可能是数字 也可能是X
            Console.WriteLine("请输入身份证号码:");
            string s = Console.ReadLine();

            Regex reg = new Regex(@"^\d{17}[\dX]$");
            //if(里面的结果是否为true)
            //reg.IsMatch(s) 最终返回一个布尔值 表示是否找到符合的元素
            if (reg.IsMatch(s))
            {
            ...
            {
                Console.WriteLine("身份证输入不正确");
            }
            */

            //题2:验证日期格式 xxxx-xx-xx格式
            Regex reg = new Regex(@"^\d{4}-\d{2}-\d{2}$");
            if (reg.IsMatch("2024-09-23"))
            {
